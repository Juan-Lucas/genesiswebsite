...
@section('page-subtitle', 'Gérez tous vos articles de blog')

@section('content')
<div class="mb-6">
    <a href="{{ route('admin.articles.create') }}" class="inline-flex items-center px-4 py-2 bg-gradient-genesis text-white font-semibold rounded-lg hover:opacity-90 transition">
                ...
                <tr class="hover:bg-gray-50">
                    <td class="px-6 py-4">
                        <div class="flex items-center">
                            @if ($article->image)
                                <img src="{{ Storage::url($article->image) }}" class="w-12 h-12 rounded object-cover mr-3">
                            @endif
                            <div>
                    ...
                    <td class="px-6 py-4 whitespace-nowrap">
                        @if ($article->category)
                            <span class="px-2 py-1 text-xs font-medium bg-gray-100 rounded">{{ $article->category }}</span>
                        @else
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        <div class="flex items-center justify-end space-x-2">
                            @if ($article->published_at)
                                <a href="{{ route('blog.show', $article) }}" target="_blank" class="text-gray-600 hover:text-gray-900">
                            ...
                            <form action="{{ route('admin.articles.toggle-publish', $article) }}" method="POST" class="inline">
                                @csrf
                                <button type="submit" class="{{ $article->published_at ? 'text-orange-600 hover:text-orange-900' : 'text-green-600 hover:text-green-900' }}">
                                    @if ($article->published_at)
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            @empty
                <tr>
                    <td colspan="6" class="px-6 py-12 text-center text-gray-500">
                        ...
                        <a href="{{ route('admin.articles.create') }}" class="mt-4 inline-flex items-center px-4 py-2 bg-gradient-genesis text-white font-semibold rounded-lg hover:opacity-90 transition">
